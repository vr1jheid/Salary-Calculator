...
var AvgCheckValue;
(function (AvgCheckValue) {
    AvgCheckValue[AvgCheckValue["low"] = 0] = "low";
    AvgCheckValue[AvgCheckValue["mid"] = 1] = "mid";
    AvgCheckValue[AvgCheckValue["high"] = 2] = "high";
})(AvgCheckValue || (AvgCheckValue = {}));
/* ************************ CHECK URL ************************ */
...
periodStartDateInput.value = dateToStr(new Date(Number(currentDate.year), Number(currentDate.month) - 1, 1));
periodEndDateInput.value = dateToStr(new Date(Number(currentDate.year), Number(currentDate.month), 0));
// main variables
...
feedbacksField.addEventListener("click", (event) => {
    ...
    if (button.getAttribute("id") === "decrease-btn" &&
...
collectorForm.addEventListener("submit", (event) => {
    ...
    const statsForToday = stats[`day_${getSelectedDate().day}`];
...
presentorForm.addEventListener("submit", async (e) => {
    ...
    const filteredStats = stats.filter((elem) => Number.parseInt(elem.monthId) >= periodStart.monthId &&
    ...
    filteredStats.forEach(({ id, monthId, ...days }) => {
        ...
        Object.entries(days)
            ...
            const avgCheckTitle = createElement("p", "avg-check-title", avgCheckContainer);
            avgCheckTitle.innerHTML = `Средний чек:`;
            const avgCheckValue = createElement("p", "avg-check-value", avgCheckContainer);
            avgCheckValue.innerHTML = `${getAvgCheckByLabel(data.avgCheck.value)}`;
            // feedbacks
            const feedbacksContainer = createElement("p", "feedbacks", optionalData);
            const feedbacksTitle = createElement("p", "feedbacks-title", feedbacksContainer);
            feedbacksTitle.innerHTML = `Отзывы:`;
            const feedbacksValue = createElement("p", "feedbacks-value", feedbacksContainer);
            feedbacksValue.innerHTML = `${data.feedbacks}шт.`;
            const deleteBtn = createElement("button", "delete-day-btn", optionalData);
    ...
    if (presentorForm.getBoundingClientRect().top > 0) {
        const statsPosition = presentorForm.getBoundingClientRect().top + window.scrollY;
        window.scrollTo({
            top: statsPosition,
            left: 0,
            behavior: "smooth",
...
function createModal() {
    ...
    const modalButtonsContainer = createElement("div", "modal-buttons-contaner", modalContent);
    const cancelBtn = createElement("button", "modal-cancel-btn", modalButtonsContainer);
    cancelBtn.innerHTML = "Отмена";
    const confirmBtn = createElement("button", "modal-confirm-btn", modalButtonsContainer);
    confirmBtn.innerHTML = "ОК";
    const closeModalBtn = createElement("button", "modal-close-btn", modalContent);
...
function correctStatStyle(e) {
    ...
    if (!summaryElem)
        return;
    const date = summaryElem.querySelector(".date");
    const salary = summaryElem.querySelector(".salary");
...
function dateToStr(date) {
    ...
    const currentMonth = date.getMonth() + 1 < 10
        ? "0" + (date.getMonth() + 1)
        : date.getMonth() + 1;
